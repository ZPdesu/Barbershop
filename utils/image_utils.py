...
def dilate_erosion_mask_path(im_path, seg_net, dilate_erosion=5):
    ...
    IM1 = (
        BicubicDownSample(factor=2)(
            torchvision.transforms.ToTensor()(Image.open(im_path))[:3]
            .unsqueeze(0)
            .cuda()
        ).clamp(0, 1)
        - seg_mean
    ...
    return (
        torch.from_numpy(hair_mask_dilate).float(),
        torch.from_numpy(hair_mask_erode).float(),
    )
